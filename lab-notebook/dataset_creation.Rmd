...
data_for_nonmem <- left_join(from_sim, from_orig) %>% 
  group_by(ID) %>%
  fill(WT:CMT) %>%
  mutate(EVID = ifelse(DV == 0, 1, 0),
         DOSE = ifelse(EVID == 1, DOSE, NA),
         ADDL = ifelse(EVID == 1, ADDL, NA),
         II = ifelse(EVID == 1, II, NA)
         ) %>% 
...
```{r}
write_nonmem("../modeling/data/rich_100.csv", data_for_nonmem)
```
